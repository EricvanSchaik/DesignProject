...
#         res.append(df_rolls)
#
#     res = pd.concat(res).set_index(timestamp_col).sort_index(axis=1).sort_index(axis=0)
#
...
def windowing4(df: pd.DataFrame, cols: [str], label_col: str, timestamp_col: str, **funcs):
    """
    Windows over a DataFrame by splitting it into segments based on the label column and
    windowing over every segment separately.
        ...
        for col in cols:
            ...
            if funcs:
                for func_name, func in funcs.items():
                    ...
                    df_roll = roll.apply(func, raw=True)
                    df_roll = df_roll.rename(columns={col: new_col})
                    df_roll = df_roll[rps*2 - 1::rps]
...
def windowing5(df: pd.DataFrame, cols: [str], label_col='Label', timestamp_col='Timestamp'):
    """
    Windows over a DataFrame by splitting it into segments based on the label column and
    windowing over every segment separately.
