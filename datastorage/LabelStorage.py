...
class LabelStorage:
    ...
    def new_label(self, name, color, desc):
        try:
            self.cur.execute(self.sql_new_label, (name, color, desc))
            self.conn.commit()
    ...
    def delete_label_type(self, name):
        self.cur.execute(self.sql_del_label_type, name)
        self.cur.execute(self.sql_del_label_data_all, name)
        self.conn.commit()
    ...
    def add_label(self, time, name, sensor):
        try:
            self.cur.execute(self.sql_add_label, (time, name, sensor))
            self.conn.commit()
        except sqlite3.Error as e:
    ...
    def delete_label(self, time, sens_id):
        self.cur.execute(self.sql_del_label_data, (time, sens_id))
        self.conn.commit()
